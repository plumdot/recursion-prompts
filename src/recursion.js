...
var arraySum = function(array) {
  ...
  array.forEach(function(element){
    ...
    if (Array.isArray(element)) {
      output.push(arraySum(element));
    }

  });

  return output.reduce(function(sum, element){
    return sum += element;
  }, 0);
...
var range = function(x, y) {
  ...
  return output.concat(range(next, y));
...
var reverse = function(string) {
  ...
  if (flipped.length === last + 1) {
    return flipped;
  }

  flipped += string[last];
  return flipped.concat(reverse(string.slice(0, last)));
};
...
var fizzBuzz = function(n) {
	...
	let output = [];
	output.push(fizzBuzzPicker(n));
	output = fizzBuzz(n-1).concat(output);
	return output;

};

var fizzBuzzPicker = function(n) {
	if ((n % 5 === 0) && (n % 3 === 0)) {
		return 'FizzBuzz';
	} else if (n % 5 === 0) {
		return 'Buzz';
...
var rMap = function(array, callback) {
	...
	output = output.concat(rMap(array.slice(1, array.length), callback));
	return output;
};
...
var fibonacci = function(n) {
  ...
  let arrayFromRecursion = fibonacci(n-1);
  let numToConcat = arrayFromRecursion[arrayFromRecursion.length - 1] + arrayFromRecursion[arrayFromRecursion.length - 2];

  return arrayFromRecursion.concat(numToConcat);
...
var capitalizeWords = function(array) {
  ...
  let wordToCap = array.shift();
  let capitalizedWord = wordToCap.toUpperCase();

  let output = [];
  output.push(capitalizedWord);

  return output.concat(capitalizeWords(array));
};
